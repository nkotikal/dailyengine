...
# and the end-of-day recap (at the recap time). Each is
...
$startTime = "06:00"     # first tick; covers the morning send + the recap time
...
function Register-ViaCmdlet {
    ...
    Register-ScheduledTask -TaskName $taskName -Action $action -Trigger $trigger `
        -Settings $settings `
        -Description "Sends the morning digest and the end-of-day recap when each is due (checks every $intervalMin min)." `
        -Force | Out-Null
}
...
Write-Host ""
Write-Host "It sends the morning digest and the end-of-day recap when each is due."
